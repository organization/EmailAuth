		...
		switch ($type) {
			...
			case CRYPT_RSA_PUBLIC_FORMAT_PKCS1:
				...
				$components ['primes'] = array (
						1 => new BigInteger ( $this->_string_shift ( $key, $length ), 256 ) 
				);
				$this->_string_shift ( $key );
				$length = $this->_decodeLength ( $key );
				...
				$components ['coefficients'] = array (
						2 => new BigInteger ( $this->_string_shift ( $key, $length ), 256 ) 
				);
			...
			case CRYPT_RSA_PRIVATE_FORMAT_PUTTY :
				...
				$components ['primes'] = array (
						1 => new BigInteger ( $this->_string_shift ( $private, $length ), - 256 ) 
				);
				extract ( unpack ( 'Nlength', $this->_string_shift ( $private, 4 ) ) );
				if (strlen ( $private ) < $length) {
					return false;
				...
				$components ['coefficients'] = array (
						2 => new BigInteger ( $this->_string_shift ( $private, $length ), - 256 ) 
				);
	...
	function crypt_random_string($length) {
		...
		if ($crypto === false) {
			...
			switch (true) {
				...
				default :
					$crypto = $seed;
					return $this->crypt_random_string ( $length );
			}
